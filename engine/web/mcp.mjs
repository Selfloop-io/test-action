...
export async function connectPlaywrightMcp(screenshotsDir, viewport) {
  const mcp = new Client({ name: 'v2-web-engine', version: '0.0.0' });
  const args = [PLAYWRIGHT_MCP_BIN, '--isolated', '--output-dir', screenshotsDir];
  // CI runners (GitHub Actions ubuntu) have no display server — @playwright/mcp
  // launches headed by default, which hard-fails there.
  if (process.env.HEADLESS === '1') args.push('--headless');
  // Headed (non-headless) launches default to an unconstrained viewport (window size),
  // not a fixed one — without this flag the browser renders at whatever size the OS
  // opens the window, silently mismatching the size the model is told (prompts.mjs
  // VIEWPORT line) and drifting the target off any real breakpoint the app was built for.
